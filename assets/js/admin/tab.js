...
layui.define(['element', 'common'], function(exports) {
	...
	/**
	 * 按照指定标题刷新 Tab 选项卡的 iframe
	 * @param {String} 标题
	 */
	Tab.prototype.refreshTab = function(title,url) {
		var that = this;
		var _config = that.config;
		var tabIndex = that.exists(title);
		if(tabIndex > -1) {
			if(url){
				_config.elem.find('div.layui-tab-content > div').eq(tabIndex).children('iframe')[0].contentWindow.location.href = url;
				//切换到选项卡
				element.tabChange(ELEM.tabFilter, tabIndex);
			}else{
				_config.elem.find('div.layui-tab-content > div').eq(tabIndex).children('iframe')[0].contentWindow.location.reload();
